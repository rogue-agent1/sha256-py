...
def sha256(msg):
    ...
    msg=bytearray(msg);ml=len(msg)*8;msg.append(0x80)
    while len(msg)%64!=56: msg.append(0)
    msg+=struct.pack('>Q',ml)
    for i in range(0,len(msg),64):
        w=list(struct.unpack('>16L',msg[i:i+64]))
        for j in range(16,64):
            s0=rr(w[j-15],7)^rr(w[j-15],18)^(w[j-15]>>3);s1=rr(w[j-2],17)^rr(w[j-2],19)^(w[j-2]>>10)
            w.append((w[j-16]+s0+w[j-7]+s1)&0xFFFFFFFF)
        a,b,c,d,e,f,g,h=h0,h1,h2,h3,h4,h5,h6,h7
        for j in range(64):
            S1=rr(e,6)^rr(e,11)^rr(e,25);ch=(e&f)^((~e)&g);t1=(h+S1+ch+K[j]+w[j])&0xFFFFFFFF
            S0=rr(a,2)^rr(a,13)^rr(a,22);maj=(a&b)^(a&c)^(b&c);t2=(S0+maj)&0xFFFFFFFF
...
def main():
    p=argparse.ArgumentParser();p.add_argument("input",nargs="?");p.add_argument("-f","--file")
    a=p.parse_args()
    if a.file: data=open(a.file,"rb").read()
    elif a.input: data=a.input.encode()
    else: import sys;data=sys.stdin.buffer.read()
    print(sha256(data))
